...
public:
    ...
    void insert(int value) {
     ...
     void findparent(AVLNode *root, int val, int parent){
        if (root == NULL)
            return;
    ...
    void sumsiblings(AVLNode *root, int cari){
        if (root == NULL)
         return;
        ...
        if (root != NULL && root->data == cari){
            ...
            if (root->data < cari)
                sumsiblings(root->right, cari);
            else if (root->data > cari)
                sumsiblings(root->left, cari);
        }
    }

    void findsiblings2(int val){
        this->findsiblings(_root,val);
    }
